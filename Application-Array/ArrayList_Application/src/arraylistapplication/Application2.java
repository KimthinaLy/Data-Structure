...
public class Application2 extends javax.swing.JFrame {
    ...
    ArrayList <Integer> plateNum= new ArrayList();
    ArrayList <String> brand=new ArrayList();
    ArrayList <Integer> maxSpeed=new ArrayList();
    ArrayList <Double> cost=new ArrayList();
    ArrayList <String> color=new ArrayList();
    ArrayList <Integer> horsePow=new ArrayList();
...
//---------------------------ArrayTrav----------------------------------
    public void ArrayTrav() {
        ...
        for (int k = 0; k < plateNum.size(); k++) {
            model.addRow(new Object[]{plateNum.get(k), brand.get(k), maxSpeed.get(k), cost.get(k), color.get(k), horsePow.get(k)});
        }
    }
    //-----------------------Insert------------------------------------------ 

    public void insert(int i, int newPlateNum, String nBrand, int nMaxSpeed, double nCost, String nColor, int nHorsePow) {
        plateNum.add(i,newPlateNum);
        brand.add(i,nBrand);
        maxSpeed.add(i,nMaxSpeed);
        cost.add(i,nCost);
        color.add(i,nColor);
        horsePow.add(i,nHorsePow);
    }
    ...
    public int search(int findCar) {
        ...
        int LOC=plateNum.indexOf(findCar);
        return LOC;
    }

    //-------------------------Sort---------------------------
    public void impl(int PTR) {
        int tempP = plateNum.get(PTR);
        String tempB = brand.get(PTR);
        int tempMS = maxSpeed.get(PTR);
        double tempC = cost.get(PTR);
        String tempColor = color.get(PTR);
        int tempHP = horsePow.get(PTR);
        
        plateNum.set(PTR,plateNum.get(PTR+1));
        plateNum.set(PTR+1,tempP);
        brand.set(PTR,brand.get(PTR+1));
        brand.set(PTR+1,tempB);
        maxSpeed.set(PTR,maxSpeed.get(PTR+1));
        maxSpeed.set(PTR+1,tempMS);
        cost.set(PTR,cost.get(PTR+1));
        cost.set(PTR+1,tempC);
        color.set(PTR,color.get(PTR+1));
        color.set(PTR+1,tempColor);
        horsePow.set(PTR,horsePow.get(PTR+1));
        horsePow.set(PTR+1,tempHP);
        
    }

    public void sortByPlateNum() {
        for (int k = 0; k < (plateNum.size()-1); k++) {
            int PTR = 0;
            while (PTR < ((plateNum.size()-1 )- k)) {
                if (plateNum.get(PTR) > plateNum.get(PTR+1)) {
                    impl(PTR);
                }
    ...
    public void sortByMaxSpeed() {
        for (int k = 0; k < (maxSpeed.size()-1); k++) {
            int PTR = 0;
            while (PTR < ((maxSpeed.size()-1) - k)) {
                if (maxSpeed.get(PTR)> maxSpeed.get(PTR+1)) {
                    impl(PTR);
                }
    ...
    public void sortByCost() {
        for (int k = 0; k < (cost.size()-1); k++) {
            int PTR = 0;
            while (PTR < ((cost.size()-1) - k)) {
                if (cost.get(PTR)> cost.get(PTR+1)) {
                    impl(PTR);
                }
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        ...
        if (k < plateNum.size()) {
            jTextField3.setText(String.valueOf(plateNum.get(k)));
            jTextField4.setText(String.valueOf(brand.get(k)));
            jTextField5.setText(String.valueOf(cost.get(k)));
            jTextField6.setText(String.valueOf(maxSpeed.get(k)));
            jTextField7.setText(String.valueOf(color.get(k)));
            jTextField8.setText(String.valueOf(horsePow.get(k)));
            delete(k);
            ArrayTrav();
        }
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton9ActionPerformed
        ...
        if (result == -1) {
            ...
            jTextField10.setText(String.valueOf(plateNum.get(result)));
            jTextField11.setText(String.valueOf(brand.get(result)));
            jTextField12.setText(String.valueOf(maxSpeed.get(result)));
            jTextField13.setText(String.valueOf(cost.get(result)));
            jTextField14.setText(String.valueOf(color.get(result)));
            jTextField15.setText(String.valueOf(horsePow.get(result)));
        }
